<template>
  <div>
    ...
    <div class="card" style="line-height: 30px; margin-bottom: 10px">
      <div>欢迎您，<el-text type="primary" size="large">{{ user.name }}</el-text>祝您今天过得开心！</div>
    </div>
    ...
    <div class="card">
      <div style="display: flex; flex-wrap: wrap">
        <div v-for="item in data.tables" :key="item.id"
          ...
          <div><img src="@/assets/imgs/logo.png" alt="" style="width: 100px"></div>
          <div>{{ item.no }}</div>
...
const addOrder = (item) => {
  item.userId = user.id
  request.put('/tables/useTable', item).then(res => {
    if (res.code === '200') {
      router.push('/order')
    } else {
